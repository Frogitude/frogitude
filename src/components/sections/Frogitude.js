          ...
          >
            <h2 className="text-4xl md:text-5xl font-bold text-text-primary">
              <span className="text-gradient">
                {t.title || "What is Frogitude?"}
              </span>
            </h2>
            <ChevronDown
              className={`w-6 h-6 text-text-secondary transition-transform ${isOpen ? "rotate-180" : ""}`}
            />
          </button>
            ...
            <div className="space-y-6 text-text-secondary/95 leading-relaxed text-lg">
              ...
              <div>
                ...
                <ul className="space-y-2 pl-4">
                  ...
                  <li className="italic">
                    {t.example2 ||
                      "– He has such frogitude; nothing disturbs his peace."}
                  </li>
                </ul>
              </div>
                ...
                <button
                  ...
                  aria-label={
                    isPlaying
                      ...
                      : (t.audioPlay || "Play") + " audio"
                  }
                >
                  {isPlaying ? (
                    <Pause className="w-5 h-5 text-accent-lime" />
                  ) : (
                    <Play className="w-5 h-5 text-accent-lime" />
                  )}
                  <span>
                    {(isPlaying
                      ? t.audioPause || "Pause"
                      : t.audioPlay || "Play") +
                      " " +
                      (t.audioCta || "Frogitude pronunciation")}
                  </span>
                </button>
              </div>
              <div className="pt-4">
                <h3 className="text-2xl font-semibold text-text-primary mb-2">
                  {t.unityTitle || "Frogitude in Unity Development"}
                </h3>
                <p>{t.unityText}</p>
              </div>
            </div>
