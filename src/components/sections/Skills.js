...
const skillCategories = {
  game_dev: { icon: Gamepad2 },
  xr_dev: { icon: BrainCircuit },
  software_eng: { icon: Code },
...
const SkillRadarChart = ({
  ...
  <ResponsiveContainer width="100%" height={400}>
    ...
    >
      ...
      <PolarAngleAxis
        dataKey="label"
        tick={{ fill: "var(--text-secondary)", fontSize: tickFontSize }}
      />
      <PolarRadiusAxis
        angle={90}
        domain={[0, max]}
        tickCount={Math.max(3, Math.min(6, Math.ceil(max)))}
        axisLine={false}
        tickFormatter={(v) => `${v}y`}
  ...
  const yearSkills = currentSkills
    .map((s) => {
      ...
      const short =
        s.subject.length > 12
          ? s.subject.replace(
              /\b(Engine|Programming|Optimization|Architecture|Testing|Comm\.|Cloud Services)\b/g,
              (m) => {
                const map = {
                  Engine: "Eng.",
                  Programming: "Prog.",
      ...
      return {
        subject: s.subject,
        years: y,
        label: yRaw > 0 ? `${short} ${yRaw.toFixed(1)}y` : short,
      };
    })
    .filter((s) => s.years > 0);
                ...
                <div key={skill.subject} className="w-full" data-skill-item>
                  <div className="flex justify-between mb-1 text-sm">
                    <span className="text-text-primary font-medium flex items-center gap-2">
                      {skill.subject}
                      <span className="px-2 py-0.5 rounded-full text-[10px] leading-none glass-effect border border-border-primary text-text-secondary">
                        {yrs.toFixed(1)}y
                      </span>
                    </span>
                  ...
                  <div className="w-full h-2 bg-border-primary/30 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      style={{
                        boxShadow: `0 0 ${glowPx}px rgba(34,197,94,0.45)`,
                      }}
                      initial={{ width: 0 }}
                      whileInView={{
                        width: `${(yrs / SCALE_MAX_YEARS) * 100}%`,
                      }}
