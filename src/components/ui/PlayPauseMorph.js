...
export default function PlayPauseMorph({ size = 48 }) {
  ...
  const toggle = () => {
    ...
    if (isPlaying.current) {
      gsap
        ...
        .to(
          playRef.current,
          { autoAlpha: 0, scale: 0.9, duration: 0.2, ease: "power2.inOut" },
          0,
        )
        .to(
          pauseRef.current,
  ...
  return (
    <button
      onClick={toggle}
      className="inline-flex items-center justify-center rounded-full glass-effect border border-border-primary hover:border-accent-lime p-3"
    >
      <svg
        width={size}
        height={size}
