...
export const onRequestPost = async (context: any) => {
  ...
  try {
    ...
    const langIsGerman =
      /[äöüÄÖÜß]|\b(der|die|das|und|oder|mit|für|über|Projekt|Anfrage)\b/i.test(
        last,
      );
    ...
    if (!env || !env.AI) {
      return new Response(
        JSON.stringify({
          error:
            'Cloudflare Workers AI binding "AI" is missing. Add it in Pages → Settings → Functions → Bindings.',
        }),
        {
          status: 500,
          headers: {
